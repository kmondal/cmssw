...
void compareHisto(const vector<TFile*>& v, const char* folder, const char* hname) 
{
  for (uint i = 0; i < v.size(); ++i) {
    TFile* f = v[i];
    //f->cd(folder);
    if (i==0) f->cd("DQMData/Run 316218/StandaloneTrackMonitor/Run summary/highPurityTracks/");
...
void compareHisto(TCanvas* canvas, const vector<TFile*>& v, const vector<string>& lglist, const vector<string>& tokens) 
{
  ...
  TLegend *legend = new TLegend(0.6,0.8,0.70,0.9);
  ...
  TFile* f = v.at(0);
  //f->cd(folder.c_str());
  f->cd("DQMData/Run 316218/StandaloneTrackMonitor/Run summary/highPurityTracks/");
  TH1F *h = dynamic_cast<TH1F*>(gDirectory->Get(hname.c_str()));
  assert(h);
